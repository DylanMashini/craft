...
async function queryDbForRecipe(
	...
	// if (env.DB_DISABLED) return undefined;
...
async function addRecipeToDB(item1: Item, item2: Item, outputItem: Item) {
	// if (env.DB_DISABLED) return {
	// 	emoji: outputItem.emoji,
	// 	new: true,
	// };
...
const generateGPT = async (item1, item2) => {
	...
	if (name.length > 20) {
		...
		let res = await generateGPT(item1, item2);
		name = res.name;
		emoji = res.name;
	}
...
export const POST: RequestHandler = async ({ request }) => {
	...
	let { emoji, name } = await generateGPT(item1, item2);
