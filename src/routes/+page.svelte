  ...
  const checkCollisions = () => {
    for (let i = 0; i < draggedElements.length; i++) {
      ...
      for (let j = i + 1; j < draggedElements.length; j++) {
        ...
        if (
          isTouching(draggedElements[i].element!, draggedElements[j].element!)
        ) {
          return [i, j];
        }
      }
...
<div class="w-screen h-screen flex">
  ...
  <div class="w-[70%] relative">
    ...
    <div class="absolute bottom-4">
      <div class="group">
        <div class="group-hover:block hidden mb-4 w-96 border-2 rounded-md p-4">
          Welcome to Infinite Craft, a game hevily inspired by the version on <a
            href="https://neal.fun/infinite-craft/"
            class="text-blue-500 underline">neal.fun</a
          >
          <br />
          <br />
          To play, start with the fundamental building blocks, and drag and drop
          them onto the crafting area. Combine two things to make something new.
          Once you find something new, it will popup in your availible items. After
          you get the hang of things, you can try the timed mode, where you race
