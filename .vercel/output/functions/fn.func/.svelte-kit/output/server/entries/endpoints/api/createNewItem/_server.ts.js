...
import "vite";
const OPENAI_API_KEY = "REDACTED_SECRET";
const prisma = new PrismaClient();
async function queryDbForRecipe(item1, item2) {
...
const upsertItemFromDb = async (item) => {
...
async function addRecipeToDB(item1, item2, outputItem) {
...
3. The series of emojis should be the ones that most accurately depict what you are trying to repersent. It is critical that the emoji used looks like what the word is. The emoji has nothing to do with combining the two inital emojis, it should repersent what the resultant word's meaning is as closely as possible.`
      ...
      },
      {
        ...
        content: [
          {
            type: "text",
            text: item1.emoji + item1.name + " AND " + item2.emoji + item2.name
          }
        ]
      }
    ],
  ...
  if (res === null)
    return error(500);
  let emojiRegex = /^[\u{1F600}-\u{1F64F}\u{1F300}-\u{1F5FF}\u{1F680}-\u{1F6FF}\u{1F700}-\u{1F77F}\u{1F780}-\u{1F7FF}\u{1F800}-\u{1F8FF}\u{1F900}-\u{1F9FF}\u{1FA00}-\u{1FA6F}\u{1FA70}-\u{1FAFF}\u{2600}-\u{26FF}\u{2700}-\u{27BF}\u{FE00}-\u{FE0F}\u{1F1E6}-\u{1F1FF}\u{1F201}-\u{1F251}\u{1F004}\u{1F0CF}\u{1F170}-\u{1F171}\u{1F17E}-\u{1F17F}\u{1F18E}\u{3030}\u{2B50}\u{2B55}\u{2B1B}\u{2B1C}\u{25AA}\u{25AB}\u{25B6}\u{25C0}\u{25FB}-\u{25FE}]+/gu;
